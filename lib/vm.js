...
// Top
function TopDefineModule(value) {
    var target;
    var receiver = exports.getCurrentModule();

    if (receiver) {
        target = receiver.getMetaclass().getConstructor().prototype;
    } else {
        target = global;
    }

    if (target[value.name]) {
        if (!(target[value.name] instanceof Module) || SaphireClass.getType(target[value.name]) != SaphireClass.MODULE) {
            throw new TypeError(value.name + " is not a module");
        }
    } else {
        target[value.name] = new Module(value.name);
    }

    exports.applyModuleScope(target[value.name], value);
};

function TopDefineClass(superclass, value) {
    var target;
    var receiver = exports.getCurrentModule();
    ...
    if (target[value.name]) {
        ...
        target[value.name] = new Class(value.name, superclass);
    }
...
exports.initialize = function() {
	exports.defineGlobalProperty('defmod', {
		set: TopDefineModule,
		get: function() {
			return TopDefineModule;
		}
	});
	
	exports.defineGlobalProperty('defclass', {
		set: TopDefineClass,
		get: function() {
			return TopDefineClass;
		}
	});
};

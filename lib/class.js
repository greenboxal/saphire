...
var SaphireClass = function(base, type, klass) {
	...
	if (klass) {
		...
		this.klass = new Function('return function $$saphire$' + Shared.fetchObjectId() + '(){ return arguments.callee.create.apply(arguments.callee, arguments); }')();
	...
	while (klass) {
		...
		if (prop) {
			...
			if (isSetter) {
				...
				return isAccessor ? prop.get : prop.value;
			}
...
SaphireClass.initialize = function() {
	Shared.BasicObject = SaphireClass.createBasicClass('BasicObject', null).getConstructor();
	Shared.Object = SaphireClass.createNativeClass('Object', Object, Shared.BasicObject).getConstructor();
	Shared.Module = SaphireClass.createBasicClass('Module', Shared.Object).getConstructor();
	Shared.Class = SaphireClass.createBasicClass('Class', Shared.Module).getConstructor();

	SaphireClass.setClassOf(Shared.BasicObject, Shared.Class);
